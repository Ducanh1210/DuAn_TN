                            ...
                            <div class="row g-3 align-items-start">
                                ...
                                <div class="col-sm-4 col-md-5 position-relative">
                                    <div class="img-wrapper rounded overflow-hidden shadow-sm">
                                        <img src="{{ $event->featured_image ? (str_starts_with($event->featured_image, 'http') ? $event->featured_image : asset('storage/' . ltrim($event->featured_image, '/'))) : 'https://via.placeholder.com/600x400?text=No+Image' }}" alt="{{ $event->name }}" class="img-fluid w-100" style="aspect-ratio: 4/3; object-fit: cover; transition: transform 0.3s ease;">
                                    </div>
                                    @if($isHappening)
                                ...
                                <div class="col-sm-8 col-md-7">
                                    ...
                                    <p class="small fw-semibold text-danger mb-2"><i class="fa-regular fa-calendar me-1"></i> {{ $event->start_time ? $event->start_time->format('d/m/Y H:i') : '' }} - {{ $event->end_time ? $event->end_time->format('d/m/Y H:i') : '' }}</p>
                                    <p class="small text-muted mb-2"><i class="fa-solid fa-location-dot me-1"></i> {{ $event->location_text }}</p>
