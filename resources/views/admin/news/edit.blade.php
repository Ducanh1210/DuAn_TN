    ...
    <div class="form-card card">
        ...
        <div class="card-body">
            ...
            <div class="mb-0">
                ...
                <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="14" required>{{ old('content', $news->content) }}</textarea>
    ...
    <div class="form-card card mb-4">
        <div class="card-body py-3">
            <div class="d-flex flex-wrap gap-2">
                <span class="info-badge bg-light"><i class="fas fa-eye text-primary"></i> {{ number_format($news->view_count) }} lượt xem</span>
                <span class="info-badge bg-light"><i class="fas fa-user text-success"></i> {{ $news->author->display_name ?? $news->author->username ?? '—' }}</span>
                <span class="info-badge bg-light"><i class="fas fa-clock text-warning"></i> {{ $news->created_at->format('d/m/Y H:i') }}</span>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="img-preview-box mb-2" onclick="document.getElementById('featured_image').click()">
                @if($news->featured_image)
                    <img id="imgPrev" src="{{ asset('storage/' . $news->featured_image) }}">
        ...
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="status" name="status">
                    ...
                    <option value="hidden" {{ old('status',$news->status)=='hidden'?'selected':'' }}>👁️‍🗨️ Ẩn</option>
                </select>
            </div>
            <div class="mb-3">
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('admin.news.index') }}" class="btn btn-secondary flex-grow-1"><i class="fas fa-arrow-left me-1"></i>Quay lại</a>
