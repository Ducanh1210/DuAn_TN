    ...
    <div class="col-lg-8">
        <div class="card-minimal p-4 mb-4">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control form-control-sm @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $news->title) }}" required style="border-color: #e2e8f0;">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control form-control-sm" id="summary" name="summary" rows="3" style="border-color: #e2e8f0;">{{ old('summary', $news->summary) }}</textarea>
            </div>
            <div class="mb-0">
                ...
                <textarea class="form-control form-control-sm @error('content') is-invalid @enderror" id="content" name="content" rows="14" required style="border-color: #e2e8f0;">{{ old('content', $news->content) }}</textarea>
    ...
    <div class="col-lg-4">
        ...
        <div class="card-minimal p-3 mb-3">
            <div class="d-flex flex-wrap gap-2 text-muted" style="font-size: 0.775rem;">
                <div>Lượt xem: <strong class="text-dark">{{ number_format($news->view_count) }}</strong></div>
                <div>•</div>
                <div>Tác giả: <strong class="text-dark">{{ $news->author->display_name ?? $news->author->username ?? '—' }}</strong></div>
            </div>
        </div>
        ...
        <div class="card-minimal p-3 mb-3">
            ...
            <div class="img-preview-box mb-2 border rounded p-3 text-center bg-light cursor-pointer" onclick="document.getElementById('featured_image').click()" title="Click chọn ảnh" style="border-style: dashed !important; border-color: #cbd5e1 !important; min-height: 140px; cursor: pointer;">
                @if($news->featured_image)
                    <img id="imgPrev" src="{{ asset('storage/' . $news->featured_image) }}" class="rounded w-100" style="max-height: 160px; object-fit: cover;">
                    <div class="ph text-muted py-3" id="imgPh" style="display:none; font-size: 0.775rem;">
                        <div>Click đổi ảnh</div>
                    </div>
                @else
        ...
        <div class="card-minimal p-3">
            ...
            <div class="mb-3">
                ...
                <select class="form-select form-select-sm" id="status" name="status" style="border-color: #e2e8f0;">
                    ...
                    <option value="hidden" {{ old('status',$news->status)=='hidden'?'selected':'' }}>Ẩn</option>
                </select>
            </div>
            <div class="mb-3">
                <label for="published_at" class="form-label text-dark fw-medium" style="font-size: 0.825rem;">Ngày xuất bản</label>
                <input type="datetime-local" class="form-control form-control-sm" id="published_at" name="published_at" value="{{ old('published_at', $news->published_at ? $news->published_at->format('Y-m-d\TH:i') : '') }}" style="border-color: #e2e8f0;">
            </div>
