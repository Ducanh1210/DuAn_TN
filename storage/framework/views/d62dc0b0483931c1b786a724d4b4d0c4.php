...
<html>
<head>
    <title>Khám phá 360° - <?php echo e($location->name); ?></title>
    <meta charset="utf-8">
    ...
    <link rel="stylesheet" href="<?php echo e(asset('marzipano/style.css')); ?>">
...
<?php if($scenes->isEmpty()): ?>
    <div style="display:flex; justify-content:center; align-items:center; height:100vh; background:#111; color:#fff; flex-direction:column;">
        ...
        <a href="<?php echo e(url('/')); ?>" class="btn btn-outline-light mt-3">Quay lại Bản đồ</a>
    </div>
<?php else: ?>
    <a href="<?php echo e(url('/')); ?>" class="btn-back-map">
    ...
    <div class="interaction-toolbar">
        ...
        ?>
        <button class="interaction-btn <?php echo e($isFavorited ? 'active' : ''); ?>" id="btnToggleFavorite" title="Yêu thích">
            <i class="<?php echo e($isFavorited ? 'fa-solid' : 'fa-regular'); ?> fa-heart"></i>
        </button>
        <button class="interaction-btn" title="Báo cáo địa điểm" onclick="openReportModal(<?php echo e($location->id); ?>, 'Location')">
            <i class="fa-solid fa-flag"></i>
        </button>
        <button class="interaction-btn" id="btnToggleComments" title="Bình luận">
            <i class="fa-regular fa-comment-dots"></i>
            <span class="interaction-badge" id="commentsCountBadge"><?php echo e($location->comments->count()); ?></span>
        </button>
    </div>
    ...
    <div class="comments-drawer" id="commentsDrawer">
        ...
        <div class="comments-list" id="commentsList">
            <?php $__empty_1 = true; $__currentLoopData = $location->comments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="comment-item" id="comment-<?php echo e($comment->id); ?>">
                    <img src="<?php echo e($comment->user->avatar_url ? (str_starts_with($comment->user->avatar_url, 'http') ? $comment->user->avatar_url : asset('storage/' . $comment->user->avatar_url)) : 'https://ui-avatars.com/api/?name='.urlencode($comment->user->display_name ?? $comment->user->username).'&background=0072FF&color=fff'); ?>" alt="<?php echo e($comment->user->display_name ?? $comment->user->username); ?>" class="comment-avatar">
                    <div class="comment-body">
                        <div class="comment-author">
                            <span><?php echo e($comment->user->display_name ?? $comment->user->username); ?></span>
                            <div class="d-flex align-items-center gap-2">
                                <span class="comment-time"><?php echo e($comment->created_at->diffForHumans()); ?></span>
                                <button class="btn btn-sm text-danger p-0 border-0 bg-transparent" title="Báo cáo bình luận" onclick="openReportModal(<?php echo e($comment->id); ?>, 'Comment')"><i class="fa-solid fa-flag" style="font-size: 12px;"></i></button>
                            </div>
                        </div>
        ...
        <div class="comment-form">
            ...
            <?php else: ?>
                <div class="auth-prompt">
                    Vui lòng <a href="<?php echo e(route('login')); ?>">đăng nhập</a> để để lại bình luận và lưu địa điểm yêu thích.
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="viewer-area">
        ...
        <a href="javascript:void(0)" id="sceneListToggle">
            <img class="icon off" src="<?php echo e(asset('marzipano/img/expand.png')); ?>">
            <img class="icon on" src="<?php echo e(asset('marzipano/img/collapse.png')); ?>">
        </a>
        ...
        <div id="sceneList">
            <ul class="scenes">
                <?php $__currentLoopData = $scenes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $scene): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="javascript:void(0)" class="scene" data-id="<?php echo e($scene->id); ?>">
                        <li class="text"><?php echo e($scene->scene_name); ?></li>
                    </a>
    ...
    <script src="<?php echo e(asset('marzipano/index.js')); ?>"></script>
    ...
    <script>
        ...
        (function() {
            const audioUrl = <?php echo json_encode(asset('storage/' . $location->audio_url), 15, 512) ?>;
            const locationName = <?php echo json_encode($location->name, 15, 512) ?>;
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const locationId = <?php echo e($location->id); ?>;
    ...
    function openReportModal(id, type) {
        const checkAuth = <?php echo e(Auth::check() ? 'true' : 'false'); ?>;
        if (!checkAuth) {
            window.location.href = "<?php echo e(route('login')); ?>";
            return;
        }
    ...
    function submitReport() {
        ...
        fetch("<?php echo e(route('client.report')); ?>", {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': csrfToken,
                'Accept': 'application/json',
...
<?php /**PATH D:\laragon\www\Du_An_TN\resources\views/client/360.blade.php ENDPATH**/ ?>