        ...
        <div class="user-sidebar collapsed" id="userSidebar">
            <div class="sidebar-header">
                ...
                <a href="<?php echo e(route('client.profile')); ?>" class="user-info" style="text-decoration: none; color: inherit; display: flex; align-items: center; gap: 12px; width: 100%;">
                    <img src="<?php echo e(Auth::user()->avatar_url ? (str_starts_with(Auth::user()->avatar_url, 'http') ? Auth::user()->avatar_url : (str_starts_with(ltrim(Auth::user()->avatar_url, '/'), 'storage') ? asset(Auth::user()->avatar_url) : asset('storage/' . Auth::user()->avatar_url))) : 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->display_name ?? Auth::user()->username).'&background=0072FF&color=fff'); ?>" alt="User Avatar" class="user-avatar">
            ...
            <div class="news-dropdown-banner">
                <div class="banner-track">
                    <?php $__currentLoopData = $newsList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $news): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="/tin-tuc/<?php echo e($news->slug); ?>" class="banner-item">
                            <img src="<?php echo e($news->featured_image ? asset('storage/' . $news->featured_image) : 'https://placehold.co/260x150/1e293b/f8fafc?text=NEWS'); ?>"
                    ...
                    <a href="/tin-tuc/<?php echo e($firstNews->slug); ?>" class="banner-item" aria-hidden="true">
                        <img src="<?php echo e($firstNews->featured_image ? asset('storage/' . $firstNews->featured_image) : 'https://placehold.co/260x150/1e293b/f8fafc?text=NEWS'); ?>"
                            alt="<?php echo e($firstNews->title); ?> Clone">
                        <div class="banner-overlay">
        ...
        <div class="mission-widget-wrapper" id="mission-widget-wrapper">
            ...
            <div class="mission-header" id="mission-widget-header">
                ...
                <div style="display: flex; align-items: center; gap: 6px;">
                    <?php if(auth()->guard()->check()): ?>
                        <span class="badge bg-warning bg-opacity-10 text-warning" style="font-size: 0.65rem; background-color: rgba(245, 158, 11, 0.1); color: #d97706; padding: 2px 6px; border-radius: 4px; font-weight: 600;" id="widgetPoints"><?php echo e(Auth::user()->points); ?> điểm</span>
                    <?php endif; ?>
            ...
            <div class="mission-list-container" style="background: #ffffff; padding: 12px; border-top: 1px solid #f3f4f6; transition: max-height 0.3s ease;">
                    ...
                    <div class="d-flex flex-column gap-2" style="max-height: 220px; overflow-y: auto; scrollbar-width: none; display: flex; flex-direction: column; gap: 8px;">
                        ...
                        <div class="widget-mission-item" style="font-size: 0.72rem; padding: 6px 8px; border: 1px solid #f3f4f6; border-radius: 6px; background-color: #f9fafb; display: flex; flex-direction: column; text-align: left; gap: 4px;">
                            <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                                ...
                                <?php
                                    $sessionPoints = \App\Models\PointTransaction::where('user_id', Auth::id())
                                        ...
                                        ->sum('amount');
                                ?>
                                <span class="text-primary fw-bold" id="widgetSessionText" style="font-size: 0.68rem; font-weight: 700;"><?php echo e($sessionPoints); ?>/60 phút</span>
                            </div>
                            <div class="progress" style="height: 4px; margin-bottom: 2px; background-color: #e5e7eb; border-radius: 4px; overflow: hidden; width: 100%;">
                                <div id="widgetSessionBar" class="progress-bar bg-primary" role="progressbar" style="width: <?php echo e(min(100, ($sessionPoints / 60) * 100)); ?>%; background-color: var(--primary); height: 100%;" aria-valuenow="<?php echo e($sessionPoints); ?>" aria-valuemin="0" aria-valuemax="60"></div>
                            </div>
                        </div>
                ...
                <?php else: ?>
                    <div class="text-center py-2" style="text-align: center;">
                        ...
                        <a href="<?php echo e(route('login')); ?>" class="btn btn-primary btn-xs px-3 py-1 rounded-pill fw-semibold" style="font-size: 0.68rem; background-color: var(--primary); color: white; padding: 4px 12px; border-radius: 20px; text-decoration: none; display: inline-block;">Đăng nhập</a>
                    </div>
                <?php endif; ?>
            </div>
        </div>
            ...
            <div class="featured-loc-scroll">
                <?php $__currentLoopData = $locations->sortByDesc('view_count')->take(10); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $loc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="#" class="featured-loc-card" onclick="flyToLocation(<?php echo e($loc->id); ?>); return false;">
                        <img src="<?php echo e($loc->thumbnail_url ?: 'https://placehold.co/300x200/1e3a8a/ffffff?text=No+Image'); ?>"
                            alt="<?php echo e($loc->name); ?>" class="featured-loc-img">
...
</html><?php /**PATH D:\laragon\www\Du_An_TN\resources\views/client/home.blade.php ENDPATH**/ ?>