...
# Kiểm tra kết nối tới huggingface.co với timeout siêu nhanh để tránh treo khi khởi động
def _check_network():
    try:
...
network_thread = threading.Thread(target=_check_network)
network_thread.daemon = True
network_thread.start()
network_thread.join(timeout=1.2)
...
filtered_backbones = {
    "VieNeu-TTS-v3-Turbo (Thử nghiệm)": {
        ...
        "description": "🆕 v3 Turbo (early access) — 48kHz."
    }
}
...
def restore_ui_state():
    ...
    global model_loaded, PRESET_VOICES_CACHE, CONV_VOICES_CACHE, current_backbone, tts
    ...
    conv_tab_update = gr.update()
    slot_updates = [gr.update()] * MAX_SPEAKERS
    
    if model_loaded:
        if PRESET_VOICES_CACHE:
            default_v = tts._default_voice if tts else None
            is_tuple = (len(PRESET_VOICES_CACHE) > 0 and isinstance(PRESET_VOICES_CACHE[0], tuple))
            voice_values = [v[1] for v in PRESET_VOICES_CACHE] if is_tuple else PRESET_VOICES_CACHE
            if not default_v and voice_values:
                default_v = voice_values[0]
            voice_update = gr.update(choices=PRESET_VOICES_CACHE, value=default_v, interactive=True)
            ...
            msg_err = "⚠️ Không tìm thấy file voices.json. Vui lòng dùng Tab Voice Cloning."
            voice_update = gr.update(choices=[msg_err], value=msg_err, interactive=False)
            tab_p = gr.update(visible=True)
            tab_c = gr.update(visible=True)
            tab_sel = gr.update(selected="preset_mode")
            mode_state = "preset_mode"
            
        # Conversation tab visibility
        is_v2 = (current_backbone == "VieNeu-TTS-v2 (GPU)" or current_backbone == "VieNeu-TTS-v2 (CPU)")
        is_v3_conv = "v3" in (current_backbone or "").lower()
        conv_tab_update = gr.update(visible=is_v2 or is_v3_conv)
        
        if CONV_VOICES_CACHE:
            slot_updates = [gr.update(choices=CONV_VOICES_CACHE)] * MAX_SPEAKERS
...
def autoload_default_model():
    global tts, current_backbone, current_codec, model_loaded, PRESET_VOICES_CACHE, CONV_VOICES_CACHE, using_lmdeploy
    ...
    print(f"📦 Auto-loading default model on startup: {default_backbone} with codec {default_codec}...")
    try:
        backbone_config = BACKBONE_CONFIGS[default_backbone]
        codec_config = CODEC_CONFIGS[default_codec]
        
        backbone_device = "gpu" if HAS_GPU else "cpu"
        codec_device = "gpu" if HAS_GPU and "ONNX" not in default_codec else "cpu"
        
        if "v3-Turbo" in default_backbone:
            v3_device = "cpu" if str(backbone_device).lower() == "cpu" else "auto"
            tts = Vieneu(
                mode="v3turbo",
                backbone_repo=backbone_config["repo"],
        ...
        elif "v2-Turbo" in default_backbone:
            mode = "turbo_gpu" if "GPU" in default_backbone else "turbo"
            tts = Vieneu(
                mode=mode,
                backbone_repo=backbone_config["repo"],
                decoder_repo=codec_config["repo"],
        ...
        print("✅ Default model loaded successfully on startup!")
    except Exception as e:
        print(f"⚠️ Failed to auto-load default model: {e}")

# Run autoload on startup
autoload_default_model()
...
with gr.Blocks(theme=theme, css=css, title="VieNeu-TTS", head=head_html) as demo:
    ...
    with gr.Column(elem_classes="container"):
        gr.HTML("""
<div class="header-box" style="padding: 10px; margin-bottom: 10px; text-align: center;">
    <h1 class="header-title" style="font-size: 2rem; margin: 0; display: inline-flex; align-items: center; justify-content: center; gap: 8px;">
        ...
        # --- CONFIGURATION ---
        with gr.Accordion("⚙️ Cấu hình Model & Thiết bị", open=False, visible=False):
            with gr.Row():
        ...
        with gr.Row(elem_classes="workspace-card"):
            # --- INPUT (LEFT COLUMN) ---
            ...
            # --- OUTPUT (RIGHT COLUMN) ---
            with gr.Column(scale=2, elem_classes="output-column-card"):
        ...
        def on_backbone_change(choice):
            ...
            if is_v3:
                ...
                default_custom_codec = "NeuCodec (ONNX)" if not HAS_GPU else "NeuCodec (Distill)"
                codec_update = gr.update(value=default_custom_codec, interactive=True)
                text_update = gr.update(value=DEFAULT_TEXT_GPU)
                temp_update = gr.update(value=0.7)
            else:
